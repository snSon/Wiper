...
from JetDehaze.model import JetDehazeNet
from MSBDN.MSBDN import Net as MSBDN
from FFA.ffa_net import FFA
from AOD.net import dehaze_net as DehazeNet
...
def get_gpu_usage():
    try:
        # Run tegrastats and read only the first line
        proc = subprocess.Popen(['tegrastats'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
        output = proc.stdout.readline()
        proc.terminate()  # Stop tegrastats after first line



        # Updated regex to support formats like "GR3D_FREQ 0%@[0]"
        ...
        if match:
            ...
            print("[WARN] GPU usage not found in tegrastats output.")
            return 0, 0
    except Exception as e:
        print(f"[ERROR] Failed to get GPU usage from tegrastats: {e}")
        return 0, 0
def get_cpu_memory():
