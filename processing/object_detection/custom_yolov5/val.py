...
def run(
    ...
    callbacks=Callbacks(),
    compute_loss=None,
    dehaze=False, # apply dehazing preprocessing (default: False)
):
    """
    ...
    latency = t[0] + t[1] + t[2]
    fps = 1000 / latency
    summary_file = ROOT / "runs" / "val" / "val_log.txt"
    with open(summary_file, "a") as f:
        f.write(f"\n--- Experiment: {save_dir.name} start ---\n")
        f.write(f"Precision:       {mp:.4f}\n")
        f.write(f"Recall:          {mr:.4f}\n")
        f.write(f"mAP@0.5:         {map50:.4f}\n")
        f.write(f"mAP@0.5:0.95:    {map:.4f}\n")
        f.write(f"Pre-process:     {t[0]:.2f} ms\n")
        f.write(f"Inference:       {t[1]:.2f} ms\n")
        f.write(f"NMS:             {t[2]:.2f} ms\n")
        f.write(f"Total latency:   {latency:.2f} ms\n")
        f.write(f"FPS:             {fps:.2f}\n")
        f.write(f"--- Experiment: {save_dir.name} end ---\n")
...
def parse_opt():
    ...
    parser.add_argument("--dehaze", action="store_true", help="apply dehazing preprocessing (default: False)") # dehazed option
